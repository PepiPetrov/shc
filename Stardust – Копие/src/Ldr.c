 ...
 *  module base pointer
 */
FUNC PVOID LdrModulePeb(_In_ ULONG Hash)
{
  STARDUST_INSTANCE
  ...
  for (; Head != Entry; Entry = Entry->Flink)
  {
    ...
    if (HashString(Data->BaseDllName.Buffer, Data->BaseDllName.Length) ==
        Hash)
    {
      return Data->DllBase;
    }
 ...
 */
FUNC PIMAGE_NT_HEADERS LdrpImageHeader(_In_ PVOID Image)
{
...
FUNC PVOID LdrFunction(_In_ PVOID Library, _In_ ULONG Function)
{
  ...
  //
  ExpDir = C_PTR(Library + NtHeader->OptionalHeader
  ...
  for (DWORD i = 0; i < ExpDir->NumberOfNames; i++)
  {
    ...
    {
      STARDUST_INSTANCE
      PCHAR *tokens = StringTokenA(C_PTR(Address), ".");
      PCHAR ForwarderModuleName = tokens[0];
      PCHAR ForwarderFunctionName = tokens[1];
      Address = LdrFunctionString(LdrModuleLoad(ForwarderModuleName),
...
FUNC PVOID LdrFunctionString(PVOID Module, PCHAR Function)
{
  return LdrFunction(Module, HASH_STR(Function));
}

FUNC PVOID LdrModuleLoad(LPSTR ModuleName)
{

  STARDUST_INSTANCE
  UNICODE_STRING UnicodeString = {0};
  WCHAR NameW[260] = {0};
  PVOID Module = LdrModulePeb(HashString(ModuleName, 0));
  ...
  CustomRtlInitUnicodeString(&UnicodeString, NameW);

  Instance()->Win32.LdrLoadDll(NULL, 0, &UnicodeString, &Module);

  MmZero(NameW, sizeof(NameW));
  MmZero(&UnicodeString, sizeof(UnicodeString));
